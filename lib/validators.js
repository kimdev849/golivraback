...
function validateAddress(value, required = true) {
  ...
  // Symboles avant la première lettre (les chiffres de numéro de rue restent
  // autorisés : « 12 rue de la Paix », « PK 45 »… mais pas « @##fff » ni
  // « 555@#$$kk »).
  const firstLetterIdx = v.search(/\p{L}/u);
  const prefix = firstLetterIdx === -1 ? v : v.slice(0, firstLetterIdx);
  const leadingGarbage = prefix.replace(/\d/g, '').length;
